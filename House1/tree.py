...
for day in range(30):
    ...
    f = open(f"DAY_{day+1}.csv", "a")
    f.write(output)
    f.close()
    print("=============================\nDay: "+str(day+1))
    pima = pd.read_csv(f"DAY_{day+1}.csv")
    ...
    dataset = pd.read_csv(f"DAY_{day+1}.csv", header=None, names=col_names)
    print(dataset)
